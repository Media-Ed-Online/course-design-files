    ...
    $(this).find(BOOK).each(function(index) {
      if ($(this).find('p.instancename:contains("Level ' + s.charAt(i) + '")').length > 0) {
        ...
        $(this).each(function(index) {
          if ($(BOOK).find('.snap-header-card .snap-header-card-icons .snap-asset-completion-tracking img:not(.moved)').first()) {
            let oldIcon = $(this).parentsUntil('li.modtype_book').addClass('moved')
